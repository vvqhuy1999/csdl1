...
select nv.manv, nv.ten, nv.dienthoai, nv.luong
from nhanvien nv;

-- Hien thi thong tin cua ban du an
select *
from duan;
...
select * from nhanvien where noisinh like 'quan 1';
-- hien thi manv, hotenlot, ten thanh cot hovaten cua bang nhan vien
select concat(hotenlot,' ',ten) as 'Ho va ten'
from nhanvien;
...
select distinct concat(hotenlot,' ',ten) as 'Ho va ten'
from nhanvien
where luong > 15000000;
...
select * from nhanvien where Ten like 'H%';
-- Hiển thị họ, tên nhân viên có lương nằm trong khoảng 10.000.000 đến 18.000.000
select concat(hotenlot,' ',ten) as 'Ho va ten' ,luong
from nhanvien
where luong > 10000000 and luong <180000000;
-- Hiển thị thông tin của các phòng ban có chứa chuỗi ‘wincom’
select * from phongban
where tenpb like '%wincom';
-- Hiển thị thông tin họ va tên, lương của nhân viên có lương thấp hơn 15000000 và mã phòng ban là ‘PB002’
select concat(hotenlot, ' ' , ten) as 'Ten moi', luong
from nhanvien
where luong<15000000 and mapb like 'pb02';
select * from nhanvien
...
--  Hiển thị danh sách các phòng ban không có nhân viên tham gia dự án.
